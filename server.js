...
app.use(cors({
  origin: (origin, callback) => {
    if (!origin || allowedOrigins.some(o => origin.trim() === o.trim())) callback(null, true);
    else callback(new Error('CORS geblokkeerd: ' + origin));
  },
  methods: ['GET', 'POST'],
...
app.post('/api/chat',
  ...
  async (req, res) => {
    ...
    if (!KEY) return res.status(503).json({ error: 'AI_OFFLINE', message: 'Geen API key.' });
    ...
    try {
        ...
        for (const line of dec.decode(chunk, { stream: true }).split('\n')) {
          ...
          try {
            ...
            if (p.type === 'content_block_delta' && p.delta?.text) {
              res.write('data: ' + JSON.stringify({ choices: [{ delta: { content: p.delta.text } }] }) + '\n\n');
            }
      ...
      res.write('data: ' + JSON.stringify({ error: 'AI_OFFLINE' }) + '\n\n');
      res.end();
    }
  }
);
...
// ── Pipedrive leads ────────────────────────────────────────────────────────
app.post('/api/leads',
  [body('name').isString().trim().notEmpty(), body('email').isEmail().normalizeEmail(), body('phone').optional(), body('company').optional().isString(), body('message').optional().isString(), body('chatHistory').optional().isArray()],
  async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
